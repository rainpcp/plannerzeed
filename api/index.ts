...
app.post("/api/auth/login", zValidator("json", loginSchema), async (c) => {
  ...
  ).bind(email, password).first() as { id: string; name: string; email: string } | null;
...
app.get("/api/tasks", auth, async (c) => {
  ...
  return c.json(tasks.results.map((t: any) => ({
...
app.post("/api/tasks", auth, zValidator("json", taskSchema), async (c) => {
  const userId = c.get("userId");
...
app.put("/api/settings", auth, async (c) => {
  ...
  if (fields.length > 1) {
    ...
    if (existing) {
      ...
      values.unshift(randomUUID(), userId);
      await c.env.DB.prepare("INSERT INTO settings (id, user_id, " + fields.map(f => f.split(" = ")[0]).join(", ") + ") VALUES (?, ?, " + fields.map(() => "?").join(", ") + ")").bind(...values).run();
    }
  }
  return c.json({ success: true });
});
